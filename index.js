...
if (!supabaseUrl || !supabaseKey) {
    console.error("❌ ስህተት: SUPABASE_URL ወይም SUPABASE_KEY በ Render Environment Variables ላይ አልተገኘም!");
}

const supabase = createClient(supabaseUrl, supabaseKey);

// 3. የቦት Setup
// ማሳሰቢያ፡ Token በ Environment Variable (process.env.BOT_TOKEN) ቢቀመጥ ይመከራል
...
bot.start(async (ctx) => {
    ...
    try {
        ...
        if (error) {
            ...
            return ctx.reply("❌ የዳታቤዝ ግንኙነት ችግር አጋጥሟል። እባክዎ ትንሽ ቆይተው ይሞክሩ።");
        }
...
// ቦቱን ማስነሻ
bot.launch().then(() => {
    console.log("Ardi Bingo Bot is LIVE with Supabase on Render!");
}).catch((err) => {
