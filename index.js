...
bot.action('deposit', (ctx) => {
    ctx.answerCbQuery();
    const userId = ctx.from.id;
    players[userId].state = 'WAITING_FOR_DEPOSIT';
    saveToDB();
    return ctx.replyWithPhoto({ url: CBE_LOGO }, {
        caption: '📥 <b>ገንዘብ ገቢ ለማድረግ (Deposit)</b>\n\nእባክዎ ማስገባት የሚፈልጉትን መጠን በቁጥር ብቻ ይላኩ።\n(ምሳሌ፦ 100)\n\n<i>ዝቅተኛው የዲፖዚት መጠን 50 ETB ነው።</i>',
        parse_mode: 'HTML'
    });
});
...
bot.action('leaderboard', (ctx) => {
    ...
    sorted.forEach((p, i) => {
        list += `${medals[i]} ID: ${p.playerId || "N/A"} - ${p.gamesPlayed || 0} ጨዋታ\n`;
    });
...
bot.on('text', async (ctx) => {
    ...
    if (players[userId]?.state === 'WAITING_FOR_DEPOSIT') {
        if (!isNaN(msgText) && parseInt(msgText) >= 50) {
            ...
            return ctx.reply("⚠️ እባክዎ ከ 50 በላይ የሆነ ቁጥር ብቻ ያስገቡ።");
        }
    }
});
...
bot.on('web_app_data', async (ctx) => {
    try {
        ...
        return bot.telegram.sendMessage(ADMIN_ID, 
            `🔔 *አዲስ የክፍያ ጥያቄ*\n\n👤 ተጠቃሚ ID: ${players[userId].playerId || "N/A"}\n💰 መጠን: *${amount} ETB*\n🏦 ባንክ: *${webData.bank}*\n📝 *SMS:* \`${webData.message}\``, 
            {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([
